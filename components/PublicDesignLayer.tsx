...
export default function PublicDesignLayer(){
  ...
  useEffect(()=>{let alive=true;const apply=(j:DesignSettings)=>{if(!alive)return;setDesign(j);try{localStorage.setItem("bf_platform_design_cache",JSON.stringify(j))}catch{}const root=document.documentElement;root.style.setProperty("--bf-global-bg-image",`url("${(j.background_url||"/brand/beautyflow-background.png").replace(/"/g,"%22")}")`);root.style.setProperty("--bf-global-bg-position",j.background_position||"center top");root.style.setProperty("--bf-global-overlay","0");document.body.dataset.bfDesignLoaded="1"};
  try{const cached=localStorage.getItem("bf_platform_design_cache");if(cached)apply(JSON.parse(cached))}catch{}
  const load=async()=>{try{const r=await fetch(`/api/public/design?t=${Date.now()}`,{cache:"no-store"});if(!r.ok)return;apply(await r.json() as DesignSettings)}catch{}};load();
  const onDesign=(e:Event)=>{const d=(e as CustomEvent<DesignSettings>).detail;if(d)apply(d);else load()};window.addEventListener("bf-design-updated",onDesign as EventListener);window.addEventListener("storage",onDesign as EventListener);return()=>{alive=false;window.removeEventListener("bf-design-updated",onDesign as EventListener);window.removeEventListener("storage",onDesign as EventListener)}},[path]);
  ...
  const content=<>{["background","behind","front"].map(layer=><div key={layer} className={`bf-runtime-frame-layer bf-layer-${layer}`} aria-hidden="true">{frames.filter(f=>(f.layer||"behind")===layer&&f.media_url).map(f=>{const basis=Math.max(600,Number(f.design_height)||2200);return <div key={f.id} className={`bf-runtime-frame shape-${f.shape||"rounded"} anim-${f.animation||"none"}`} style={{left:`${f.x}%`,top:`${(f.y/100)*basis}px`,width:`${f.width}%`,height:`${(f.height/100)*basis}px`}}>{f.media_type==="video"?<video src={f.media_url} autoPlay muted loop playsInline style={{objectFit:f.fit||"cover",objectPosition:`${f.media_x??50}% ${f.media_y??50}%`,transform:`scale(${(f.media_scale??100)/100})`}}/>:<img src={f.media_url} alt="" draggable={false} style={{objectFit:f.fit||"cover",objectPosition:`${f.media_x??50}% ${f.media_y??50}%`,transform:`scale(${(f.media_scale??100)/100})`}}/>}</div>})}</div>)}</>;
