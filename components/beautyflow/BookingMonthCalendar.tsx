...
export default function BookingMonthCalendar({value,onChange,minDate,maxDate,salonId,serviceId,staffId}:{value:string;onChange:(v:string)=>void;minDate?:string;maxDate?:string;salonId?:string;serviceId?:string;staffId?:string}){
 const selected=parse(value),[view,setView]=useState(()=>new Date(selected.getFullYear(),selected.getMonth(),1)),[statuses,setStatuses]=useState<Record<string,DayStatus>>({}),[loading,setLoading]=useState(false);
 ...
 useEffect(()=>{if(!salonId){setStatuses({});return}const ctrl=new AbortController();setLoading(true);const qs=new URLSearchParams({salonId,year:String(view.getFullYear()),month:String(view.getMonth()+1)});if(serviceId)qs.set("serviceId",serviceId);if(staffId)qs.set("staffId",staffId);fetch(`/api/public/month-availability?${qs}`,{signal:ctrl.signal}).then(r=>r.json()).then(j=>setStatuses(j.days||{})).catch(()=>{}).finally(()=>setLoading(false));return()=>ctrl.abort()},[salonId,serviceId,staffId,view]);
 return <div className="booking-calendar" aria-label="Избери дата">
  ...
  <div className="booking-calendar-month-legend"><span className="month-free">{serviceId?"Има свободни часове":"Работен ден"}</span>{serviceId&&<span className="month-full">Напълно зает</span>}<span className="month-off">Неработен</span><span className="month-holiday">Празник</span></div>
  <div className="booking-calendar-week">{DAYS.map(x=><span key={x}>{x}</span>)}</div>
  <div className={`booking-calendar-grid ${loading?"is-loading":""}`}>{cells.map((day,i)=>{if(!day)return <span className="calendar-empty" key={`e${i}`}/>;const current=new Date(view.getFullYear(),view.getMonth(),day),dateIso=iso(view.getFullYear(),view.getMonth(),day),rangeDisabled=!!((min&&current<min)||(max&&current>max)),active=value===dateIso,today=dateIso===iso(new Date().getFullYear(),new Date().getMonth(),new Date().getDate()),status=statuses[dateIso],holiday=holidays[dateIso],disabled=rangeDisabled||status==="unavailable";return <button type="button" key={dateIso} disabled={disabled} className={`${active?"selected ":""}${today?"today ":""}${holiday?"holiday ":""}${status?`day-${status}`:"day-unknown"}`} onClick={()=>onChange(dateIso)} title={[holiday?.name,status==="available"?(serviceId?"Има свободни часове":"Работен ден"):status==="full"?"Всички часове са заети":status==="unavailable"?"Неработен ден":""].filter(Boolean).join(" · ")}><span>{day}</span>{holiday&&<small>{holiday.short}</small>}<i/></button>})}</div>
  {!serviceId&&<p className="calendar-hint">Календарът вече следва реалния работен график. Избери услуга, за да видиш кои работни дни имат свободен час за нея.</p>}
 </div>
}
